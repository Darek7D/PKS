...
void CCamera::SetPos(float x, float y, float z, float angle)
{
	// Get the current view vector (the direction we are looking)
	CVector3 vVector = m_vView - m_vPosition;
	vVector = Normalize(vVector);

	m_vPosition.x = x;		// Add our acceleration to our position's X
	m_vPosition.y = y;		// Add our acceleration to our position's Y
	m_vPosition.z = z;		// Add our acceleration to our position's Z
	m_vView.x += m_vPosition.x +sin(angle);			// Add our acceleration to our view's X
	//m_vView.y += m_vPosition.y ;			// Add our acceleration to our view's Y
	m_vView.z += m_vPosition.z +cos(angle);			// Add our acceleration to our view's Z
}

void CCamera::CheckForMovement()
{	
	// Once we have the frame interval, we find the current speed
	float speed = kSpeed * g_FrameInterval*10;
